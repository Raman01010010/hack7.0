  ...
  async function handleSubmit(){
   ...
   if(res.status==200||res.status==201||res.status==202){
    ...
    const accessToken = res?.data?.accessToken;
            ...
            const tm=res?.data
            setNewUser({"userid":tm.userid,"email":login.email,"pwd":login.pwd,roles,  accessToken: res?.data?.accessToken,picture:res?.data?.picture,username:res?.data?.username,email:res?.data?.email});
    navigate('/dashboard')}
  }
