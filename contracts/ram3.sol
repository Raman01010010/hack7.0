...
contract Complaint {
    ...
    struct Violation {
        ...
        bool resolved;
        string details; // Additional field for more details about the violation
    }
...
//     event ViolationReported(uint indexed id, address complainant, string name, string vehicleNumber, uint timestamp, string location, string date, string registeredBy, string contactNumber, string licenseNumber, ViolationType violationType);
//     event ViolationResolved(uint indexed id);
...
//     function reportViolation(string memory _name, string memory _vehicleNumber, string memory _location, string memory _date, string memory _registeredBy, string memory _contactNumber, string memory _licenseNumber, uint8 _violationType) public {
//         require(_violationType < 4, "Invalid violation type");
        
//         violationCount++;
//         violations[violationCount] = Violation(msg.sender, _name, _vehicleNumber, block.timestamp, _location, _date, _registeredBy, _contactNumber, _licenseNumber, ViolationType(_violationType), false);
//         emit ViolationReported(violationCount, msg.sender, _name, _vehicleNumber, block.timestamp, _location, _date, _registeredBy, _contactNumber, _licenseNumber, ViolationType(_violationType));
//     }
...
//     function getViolationDetails(uint _id) public view returns (address, string memory, string memory, string memory, uint, string memory, string memory, string memory, string memory, ViolationType, bool) {
//         require(_id <= violationCount, "Violation ID does not exist");
//         Violation memory violation = violations[_id];
//         return (violation.complainant, violation.name, violation.vehicleNumber, violation.location, violation.timestamp, violation.date, violation.registeredBy, violation.contactNumber, violation.licenseNumber, violation.violationType, violation.resolved);
//     }
